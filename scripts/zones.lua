...
function getMouseZone()
    -- returns the mouse zone as two strings [top/mid/bot], [l/m/r]
...
function getZonesData(list)
    ...
    for _, v in ipairs(list) do
        ...
        if sep < 1 or sep == nil then
            ...
            msg.warn("Expected: {default|{top|mid|bot|l|r|*}[-{l|m|r|*}}]: <command>")
            ...
            if posX == nil and posY ~= "default" and posY ~= "*" and posY ~= "" then
                for _, x in pairs(HORZ) do
                    if posY == x and data['mid-'..x] == nil then
                        data['mid-'..x] = cmd
                    end
                end
                for _, y in pairs(VERT) do
                    if posY == y and data[y..'-mid'] == nil then
                        data[y..'-mid'] = cmd
                    end
                end
